...
Процедура ПерезагрузитьПоследниеТестыПоИстории(Команда = Неопределено)
	...
	Если ИсторияЗагрузкиТестов.Количество() > 0 Тогда
		...
		Попытка
			ЗагрузитьТесты(ЭлементИстории.ИдентификаторЗагрузчика, ЭлементИстории.Путь, ЭтаФорма);
		Исключение
			// TODO
...
&НаКлиенте
Процедура ЗагрузитьТесты(Знач ИдентификаторВыбранногоЗагрузчика, Знач Путь, ТекФорма) Экспорт
	ИнициализироватьИндикаторВыполнения();
	
	ТекФорма.ИдентификаторЗагрузчика = ИдентификаторВыбранногоЗагрузчика;
	ФормаЗагрузчика = Плагин(ИдентификаторВыбранногоЗагрузчика);
	
	Попытка
		ТекФорма.ДеревоОтЗагрузчика = ФормаЗагрузчика.Загрузить(ТекФорма, Путь);
	Исключение
		Сообщить(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
	
	ЭлементыДереваТестов = ТекФорма.ДеревоТестов.ПолучитьЭлементы();
	ЭлементыДереваТестов.Очистить();
	ЗаполнитьДеревоТестов(ТекФорма.ДеревоТестов, ДеревоОтЗагрузчика);
	
	КоличествоТестовыхСлучаев = ЗаполнитьКоличествоТестовыхСлучаевПоВсемуДеревуТестов(ТекФорма.ДеревоТестов);
...
&НаСервере
Процедура СохранитьПоследнююНастройку()
	Попытка
		...
		ХранилищеОбщихНастроек.Сохранить(ЭтотОбъектНаСервере().Метаданные().ПолноеИмя(), , Объект.Настройки);
	Исключение
